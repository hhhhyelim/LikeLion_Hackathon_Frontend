...
const Signup = ({navigation}) => {
  ...
  useEffect(() => {
    if(didMountRef.current){
      let _errorMessage ='';
      if(!username){
        _errorMessage ='아이디를 입력하세요(최소8글자)';
      } else if(password !== ck_password){
        _errorMessage ='비밀번호가 다릅니다';
      }else if (username.length < 8) { // Add this condition
          _errorMessage = '아이디는 8글자 이상이어야 합니다.';
      } else if(!name){
  ...
  useEffect(() => {
    setDisaled(!(name && username && password && ck_password && birth && gender && phoneNumber &&!errorMessage));
  ...
  const _handleSignupButtonPress = async () => {
    ...
    try{
      ...
      const apiUrl = "http://172.20.10.8:8080/api/signup";
